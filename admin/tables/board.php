 ...
 * Boards Table class
 */
class IotTableBoard extends JTable
{
	/**
	...
	{
		parent::__construct('#__iot_board', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_iot.board'));
	}	
	...
	/**
	 * Overload the store method for the Board table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New board. A board created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(JText::_('COM_IOT_BOARD_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_iot.board.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_iot.board.'.(int) $this->$k;
	}
