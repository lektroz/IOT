...
/**
 * Data_alarms Table class
 */
class IotTableData_alarm extends JTable
{
	/**
	...
	{
		parent::__construct('#__iot_data_alarm', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_iot.data_alarm'));
	}	
	...
	/**
	 * Overload the store method for the Data_alarm table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New data_alarm. A data_alarm created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('data_alarm', 'IotTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_IOT_DATA_ALARM_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_iot.data_alarm.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_iot.data_alarm.'.(int) $this->$k;
	}
