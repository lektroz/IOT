...
/**
 * Data_inputs Table class
 */
class IotTableData_input extends JTable
{
	/**
	...
	{
		parent::__construct('#__iot_data_input', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_iot.data_input'));
	}	
	...
	/**
	 * Overload the store method for the Data_input table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New data_input. A data_input created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('data_input', 'IotTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_IOT_DATA_INPUT_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_iot.data_input.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_iot.data_input.'.(int) $this->$k;
	}
