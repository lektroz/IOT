...
/**
 * Data_outputs Table class
 */
class IotTableData_output extends JTable
{
	/**
	...
	{
		parent::__construct('#__iot_data_output', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_iot.data_output'));
	}	
	...
	/**
	 * Overload the store method for the Data_output table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New data_output. A data_output created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('data_output', 'IotTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_IOT_DATA_OUTPUT_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_iot.data_output.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_iot.data_output.'.(int) $this->$k;
	}
