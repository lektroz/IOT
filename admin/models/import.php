 ...
 * Iot Import Model
 */
class IotModelImport extends JModelLegacy
{
	 ...
	 * @var        string
	 */
	protected $_context = 'com_iot.import';
	...
	protected function populateState()
	{
		...
		$this->setState('message', $app->getUserState('com_iot.message'));
	...
	public function import()
	{
		...
		if ($package === null)
		{
			switch ($this->getType)
			{
				...
				default:
					$app->setUserState('com_iot.message', JText::_('COM_IOT_IMPORT_NO_IMPORT_TYPE_FOUND'));
		...
		if (!$package || !$package['type'])
		{
			...
			$app->setUserState('com_iot.message', JText::_('COM_IOT_IMPORT_UNABLE_TO_FIND_IMPORT_PACKAGE'));
			return false;
		}
		...
		if (!$this->setData($package,$this->dataType,$headerList))
		{
			...
			$msg = JText::_('COM_IOT_IMPORT_ERROR');
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
				$app->setUserState('com_iot.redirect_url', 'index.php?option=com_iot&view='.$back);
		...
		else
		{
			...
			$msg = JText::sprintf('COM_IOT_IMPORT_SUCCESS', $package['packagename']);
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
			    $app->setUserState('com_iot.redirect_url', 'index.php?option=com_iot&view='.$back);
	...
	{		
		...
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_WARNIMPORTFILE'), 'warning');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_NO_FILE_SELECTED'), 'warning');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_WARNIMPORTUPLOADERROR'), 'warning');
			return false;
		}
	...
	protected function _getPackageFromFolder()
	{
		...
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_PLEASE_ENTER_A_PACKAGE_DIRECTORY'), 'warning');
			return false;
		}
		...
		if (!$type)
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_PATH_DOES_NOT_HAVE_A_VALID_PACKAGE'), 'warning');
		}
		...
		{
			// set error message
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_DOES_NOT_HAVE_A_VALID_FILE_TYPE'), 'warning');
			return false;
		}
	...
	protected function _getPackageFromUrl()
	{
		...
		if (!$url)
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_ENTER_A_URL'), 'warning');
			return false;
		}
		...
		if (!$p_file)
		{
			$app->enqueueMessage(JText::_('COM_IOT_IMPORT_MSG_INVALID_URL'), 'warning');
			return false;
		}
	...
	**/
	...
	{
		if (IotHelper::checkArray($target_headers))
		{
	...
	**/
	protected function save($data,$table)
	{
		...
		if(IotHelper::checkArray($data['array']))
		{
			// get user object
			...
			if(IotHelper::checkArray($data['array']) && $user->authorise($table.'.import', 'com_iot') && $user->authorise('core.import', 'com_iot'))
			{
				// set target.
				...
				$canDo = IotHelper::getActions($table);
				...
				foreach($data['array'] as $row)
				{
					...
					if (isset($row[$id_key]) && is_numeric($row[$id_key]) && $row[$id_key] > 0)
					{
						...
						$query
							->select('version')
							->from($db->quoteName('#__iot_'.$table))
					...
					if($found && $canEdit)
					{
						...
						$query->update($db->quoteName('#__iot_'.$table))->set($fields)->where($conditions);
						$db->setQuery($query);
						$db->execute();
					}
					elseif ($canCreate)
					{
						...
						$query
							->insert($db->quoteName('#__iot_'.$table))
	...
	protected function getAliasesUsed($table)
	{
		...
		$columns = $db->getTableColumns('#__iot_'.$table);
		if(isset($columns['alias'])){
			// Create a new query object.
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('alias')));
			$query->from($db->quoteName('#__iot_'.$table));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
