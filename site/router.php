...
/**
 * Routing class from com_iot
 *
 * @since  3.3
 */
class IotRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_iot component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'board' || $view === 'io_list' || $view === 'boards' || $view === 'board' || $view === 'getdata'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'board' || $view === 'io_list' || $view === 'boards' || $view === 'board' || $view === 'getdata'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'boards':
				...
				{
					$id = $this->getVar('board', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'iot')
	{
...
function IotBuildRoute(&$query)
{
	$router = new IotRouter;
	
	return $router->build($query);
}

function IotParseRoute($segments)
{
	$router = new IotRouter;
